...
- [ ] **Regenerate the committed CV PDF for the new contact email.** The contact email changed to `roman@romanhrupskyi.com` (2026-09-08, `chore/contact-email`), but `site/public/Roman-Hrupskyi-Senior-Java-Engineer-CV.pdf` still shows `roman.grupskyi@gmail.com` in its contact sidebar. The PDF is hand-maintained (ADR-0008) — regenerate it from the "Classic" source with the new address and re-run the parity check. — owner: Roman, due: before the next launch
