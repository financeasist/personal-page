...
This feature ships zero client JavaScript; the click-tracking beacon is roadmap step 8. Hand-rolled hero, section, contact-actions, phone-chooser, and availability-block components (SAD §5) become the site's first shared primitives.
...
**Scope narrowing vs roadmap step 3 (updated 2026-09-07):** v1 is the above-the-fold scan only. Deferred to v2:
...
Roadmap step 3's scope is amended to match; nothing above the fold is dropped. The deferred sections remain **schema-additive** later (the Profile content shape does not need to change to bring them back). `ux-flows.md` and `screens.md` predate this cut and need a re-sync pass.
...
- A Recruiter can reach Roman through any primary channel — phone, email, LinkedIn, or CV — from the top of the page (one tap; when Roman publishes more than one phone line the phone action adds one tap to pick a line), on a phone or a laptop.
...
- **No below-the-fold depth in v1** — the experience timeline (was US-04 / AC-09), the selected-work section (was US-09 / AC-11 / AC-12), the pre-2017 "earlier background" line, the full skills matrix, and the recommendation / testimonial block are all deferred to v2. v1 is the above-the-fold scan only. Section composition stays additive (no schema change needed to bring them back). This narrows roadmap step 3 (see §1).
...
**Then** without scrolling, the Recruiter sees every **above-the-fold essential**: Roman's headshot, his name, his headline, his current availability status (carrying the remote-work / relocation stance), his location, his top stack (four to eight technologies), and the four contact actions (phone, email, LinkedIn, Download CV). This is the same list AC-05 enforces at build time — the render AC and the build gate point at one canonical set.
...
**Given** a Recruiter activates the phone action
**When** Roman has published more than one phone line
**Then** the action expands (a native disclosure control that needs no JavaScript) to one labelled call control per line (for example "Call — Poland" / "Call — international"), with no phone digits shown as text, and activating one starts a call to that line;
**And when** Roman has published exactly one phone line
**Then** the phone action is a single labelled call control that starts the call directly, with no disclosure step and no digits shown as text.

_(The Profile content requires at least one phone line; two or more trigger the disclosure. Line labels are Profile content, not derived from the number.)_
...
**Given** the Profile content is missing an **above-the-fold essential** — Roman's name, the headshot (image path *and* its alt text), the headline, the availability status, the location, fewer than four entries in the top stack, or any of the four contact channels (phone with ≥1 line, email, LinkedIn, CV — all required)
**When** Roman or CI builds the site
**Then** the build fails with a message naming the missing field, and the incomplete page is never published (the invariant "the above-the-fold scan is always complete" holds). This is the same canonical essentials list AC-01 renders.
...
**Then** no salary or rate expectation, no exact home address, and none of the tracker's per-recruiter link labels appear anywhere in it, and Roman's phone numbers, email address, and LinkedIn URL / handle are never rendered as visible text on the landing page — the page exposes only what a Recruiter needs to assess fit and lets them make contact through the labelled controls. (The committed CV PDF deliberately shows the contact details — that boundary is out of this criterion's scope.)
...
**Then** no phone number, email address, or profile URL appears as visible text — each is reachable only by activating its labelled control (the invariant "contact details are actionable-only on the landing page" holds; the committed CV PDF is the deliberate exception). The values do live in the controls' link attributes so the controls function; keeping them out of the HTML source entirely was considered and **rejected for v1** (ADR-0006 — no script; script-assembled-on-click is the documented fallback if scraping abuse appears).
...
- [ ] **ADR for the committed-static-CV decision.** v1 ships a hand-committed CV PDF and defers build-time `/cv` generation — this supersedes/amends **ADR-0004** (site is the single source of truth, CV generated at build). Write the ADR (Accepted), link it from ADR-0004, and record the revisit trigger (move to generation with roadmap step 4 / next version). Default now: title "CV delivery is a committed static PDF in v1"; consequences section carries the drift risk and the manual-parity mitigation. — owner: Roman + design, due: before sdd:tasks (via `/sdd:decide-adr personal-landing`)
- [ ] **`CLAUDE.md` edit.** The `## site` section states "CV PDF is generated, never committed" and describes the `postbuild` generation step as current. Reword to: v1 ships a committed static PDF under `site/public/`; generation returns with roadmap step 4. — owner: Roman, due: with the ADR above
- [ ] **Re-sync `ux-flows.md` and `screens.md`** to the narrowed scope: drop the US-04 / US-09 flows and SCR-02 / SCR-03, keep the hero + phone-chooser. — owner: whoever runs the next pipeline stage, due: before sdd:tasks
...
- [x] Above-the-fold essentials — one canonical list for AC-01 and AC-05, and a `topStack` minimum. **Resolved 2026-09-07:** the two ACs now point at one list (name, headshot + alt, headline, availability status, location, top stack 4–8, four contact channels); `topStack` is min 4 / max 8, build-enforced.
...
- [x] Single-line phone behaviour and the `phones` array minimum. **Resolved 2026-09-07:** min 1 line; one line → direct `tel:` control, two+ → native `<details>` disclosure (AC-03).
- [x] Is the LinkedIn URL covered by the visible-text ban? **Resolved 2026-09-07:** yes — AC-07 now matches AC-10 and §1 (phone, email, and LinkedIn URL / handle).
