...
- **Contact action** — one tap target (email, LinkedIn, Download CV) that opens the corresponding channel and fires a tracked click event. The three actions live in a **persistent header** (sticky, CSS-only) at the top of the page, in reach at any scroll position; on the phone viewport the header condenses to Roman's name, the email and LinkedIn actions as icon controls, and a native-`<details>` menu holding Download CV and an in-page About link (ratified 2026-09-07, divergence D-1). On the landing page the underlying email / URL is never shown as text — the action is the only way to reach it (the CV PDF is exempt). _The **phone** was removed from the landing page in v1 (2026-09-07): `contact.phones` stays a Profile field but renders only on the CV route, so there are three Contact actions on the landing page, not four._ NOT a contact form and NOT a message-sending feature.
...
- **Tagline** — an optional short passage shown below the **Headline** in the hero: a sentence or two, length-capped at ~300 characters so the above-the-fold fit holds. Absent is valid; present-but-empty is not. Landing-page only. NOT required, NOT the Headline, NOT the CV `summary` array. _(As of 2026-09-07 the Tagline absorbs what an earlier draft split into a separate "positioning block" — divergence D-10; there is one optional prose field in the hero, not two.)_
- **Positioning block** — _removed 2026-09-07 (divergence D-10). Merged into the **Tagline**, which now allows a sentence or two (~300-char cap). There is no separate `positioning` Profile field._
- **Industries list** — an optional hero-column list of the domains Roman has delivered in (iGaming, fintech, healthcare, retail), each an entry of `domain` + an optional short `note`. Content-driven (`industries` Profile field, ≤ 6 entries); optional — when absent the hero renders without that column and the build still succeeds. It is hero content but NOT an above-the-fold essential. NOT the **Experience timeline**, NOT derived from a country code or from `experience`. (Ratified 2026-09-07, divergence D-7.)
